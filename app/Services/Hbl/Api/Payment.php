...
class Payment extends ActionRequest
{
    ...
    public function ExecuteFormJose($mid, $api_key, $curr, $amt, $threeD, $success_url, $failed_url, $cancel_url, $backend_url, $data): string
    {
        $now = Carbon::now();
        ...
        $stringPayload = json_encode($payload);
        $signingKey = $this->GetPrivateKey(config('hbl.MerchantSigningPrivateKey')); //SecurityData::$MerchantSigningPrivateKey
        $encryptingKey = $this->GetPublicKey(config('hbl.PacoEncryptionPublicKey')); //SecurityData::$PacoEncryptionPublicKey
        // $signingKey = $this->GetPrivateKey(SecurityData::$MerchantSigningPrivateKey);
        // $encryptingKey = $this->GetPublicKey(SecurityData::$PacoEncryptionPublicKey);
        ...
        $response = $this->client->post('api/1.0/Payment/prePaymentUi', [
            'headers' => [
                'Accept' => 'application/jose',
                'CompanyApiKey' => config('hbl.AccessToken'), //SecurityData::$AccessToken
                // 'CompanyApiKey' => SecurityData::$AccessToken,
                'Content-Type' => 'application/jose; charset=utf-8'
            ],
            'body' => $body,
