...
{
    private static string $paymentEndpoint = "https://core.demo-paco.2c2p.com/";
    ...
    public function __construct()
    {
        ...
        if (config('hbl.EndPoint')) {
            self::$paymentEndpoint = config('hbl.EndPoint');
        };
        ...
        $this->claimCheckerManager = new ClaimCheckerManager(
            checkers: [
                ...
                new AudienceChecker(config('hbl.AccessToken')), //SecurityData::$AccessToken),
        ...
        $this->jweLoader = new JWELoader(
            ...
            headerCheckerManager: new HeaderCheckerManager(
                checkers: [
                    new AlgorithmChecker(
                        supportedAlgorithms: [SecurityData::$JWEAlgorithm, SecurityData::$JWEEncrptionAlgorithm],
                        protectedHeader: true
                    ),
    ...
    protected function EncryptPayload(string $payload, JWK $signingKey, JWK $encryptingKey): string
    {
        ...
        $jwe = $this->jweBuilder
            ...
            ->withSharedProtectedHeader([
                ...
                "kid" => config('hbl.EncryptionKeyId'), //SecurityData::$EncryptionKeyId,
                "typ" => SecurityData::$TokenType,
            ])
