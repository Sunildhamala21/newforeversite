                ...
                <div class="kt-portlet">
                    <form class="kt-form" id="add-form-page" enctype="multipart/form-data">
                        ...
                        <div class="kt-portlet__body">
                            <div class="form-group">
                                ...
                                <select class="custom-select form-control form-control-sm" name="trip_id">
                                    ...
                                    @foreach ($trips as $trip)
                                        <option value="{{ $trip->id }}" @selected($trip_faq->trip_id == $trip->id)>{{ $trip->name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <select class="custom-select form-control form-control-sm" name="faq_category_id">
                                    ...
                                    @foreach ($faqCategories as $category)
                                        <option value="{{ $category->id }}" @selected($trip_faq->faqCategory && $trip_faq->faqCategory->id === $category->id)>
                            ...
                            <div class="form-group">
                                <label>Title</label>
                                <input type="text" value="{{ $trip_faq->title }}" name="title" class="form-control"
                                    placeholder="" required>
                            </div>
                            <div class="form-group">
                                <label>Faq</label>
                                <textarea name="description" class="editor">{{ $trip_faq->description }}</textarea>
                            </div>
                        </div>
            ...
            function handleRegionForm(form) {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if (res.status === 1) {
                            location.href =
                                '{{ route('admin.trip-faqs.faqs', ['tripId' => $trip_faq->trip_id]) }}';
                        }
                    }
                });
            }
