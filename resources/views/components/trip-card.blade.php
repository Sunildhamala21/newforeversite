...
<div class="flex flex-col bg-white shadow-md tour">
    <div class="top">
        <a href="{{ route('front.trips.show', $trip) }}">
            <img src="{{ $trip->mediumImageUrl }}" alt="{{ $trip->name }}" width="400" height="250" loading="lazy"
                class="object-cover">
        </a>
        <div class="top__overlay">
            ...
            <div>
                ...
                <span class="ml-1">{{ $trip->difficulty }}</span>
            </div>
        </div>
    </div>
    <div class="offer bg-accent text-dark">{{ $trip->best_value }}</div>
    <div class="flex flex-col justify-between bottom flex-grow-1">
        <div class="flex flex-col p-4 flex-grow-1">
            ...
            <div class="flex justify-between">
                ...
                @if ($trip->reviews_count > 0)
                    <div class="flex items-center justify-center gap-2">
                        ...
                        <span class="text-xs">({{ $trip->reviews_count }})</span>
                    </div>
                @endif
            </div>
            ...
            <a href="{{ route('front.trips.show', $trip) }}" class="mb-4 flex-grow-1">
                <div>
                    <span class="mb-2 text-2xl text-gray-600 font-display">{{ $trip->name }}</span> -
                    <span> {{ $trip->duration }} {{ $trip->duration > 1 ? 'days' : 'day' }}</span>
                </div>
            </a>
                ...
                @if ($trip->cost)
                    <div class="relative price">
                        ...
                        <div>
                            ...
                            <span
                                class="ml-1 text-lg text-gray-500">{{ $trip->offer_price ? number_format($trip->offer_price) : number_format($trip->cost) }}
                            </span>
                            @if ($trip->offer_price)
