<div x-data="{
    ...
    getFilteredOptions(query) {
        this.options = this.allOptions.filter(
            option => option.label.toLowerCase().includes(query.toLowerCase()) || option.phoneCode.includes(query.toLowerCase()),
        );
...
}" class="flex flex-col w-full max-w-lg gap-1">
    <label for="phoneNumber" class="w-fit pl-0.5 text-sm">Phone Number</label>
    <div class="relative flex w-full"
         x-on:keydown="handleKeydownOnOptions($event)"
         x-on:keydown.esc.window="isOpen = false, openedWithKeyboard = false"
         x-init="(options = allOptions), setSelectedOption(options[222])">

        <!-- Country Selector  -->
        <div>
            <!-- Trigger Button  -->
            <button type="button"
                    class="inline-flex items-center justify-between h-full gap-2 px-4 py-3 font-medium tracking-wide transition bg-white border border-r-0 border-gray-300 rounded-l-lg w-max whitespace-nowrap border-outline hover:opacity-75 focus-visible:outline-hidden focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
                    role="combobox"
                ...
                <img class="h-3.5 w-5"
                     x-bind:alt="selectedOption.value"
            ...
            <div x-show="isOpen || openedWithKeyboard"
                 id="countriesList"
                 class="absolute left-0 z-10 w-full overflow-hidden bg-white border border-gray-300 rounded-lg top-11 border-outline"
                 role="listbox"
                ...
                <div class="relative">
                    <svg xmlns="http://www.w3.org/2000/svg"
                         ...
                         fill="none"
                         stroke-width="1.5"
                         class="absolute -translate-y-1/2 left-4 top-1/2 size-5 "
                         aria-hidden="true">
                    ...
                    <input type="text"
                           class="w-full border-b border-gray-300 py-2.5 pl-11 pr-4 text-sm focus:outline-hidden focus-visible:border-primary disabled:cursor-not-allowed disabled:opacity-75 "
                           {{-- name="searchField" --}}
                           aria-label="Search"
                ...
                <ul class="flex flex-col overflow-y-auto max-h-44">
                    ...
                    <template x-for="(item, index) in options" x-bind:key="item.value">
                        <li class="inline-flex justify-between gap-6 px-4 py-2 text-sm focus-visible:outline-hidden "
                            role="option"
        ...
        <input id="phoneNumber"
               type="tel"
               class="w-full px-3 py-3 border border-gray-300 rounded-l-none rounded-r-lg border-outline focus:outline-hidden focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary disabled:cursor-not-allowed disabled:opacity-75"
               x-ref="phoneNumber"
               name="phone"
               autocomplete="phone"
               placeholder="" />
